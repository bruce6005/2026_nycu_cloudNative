...
test.beforeEach(async ({ page }) => {
  ...
  await page.route('**/src/App.tsx*', async route => {
    ...
    let patchedText = text.replace(/useState(<[^>]*>)?\(\s*null\s*\)/, `useState(${JSON.stringify(mockUser)})`);
    // 同時預設顯示 approval 分頁
    patchedText = patchedText.replace(/useState<Page>\(\"request\"\)/, `useState<Page>(\"approval\")`);
