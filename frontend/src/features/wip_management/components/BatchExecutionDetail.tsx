...
const isRunningStatus = (status: string) => {
  return status === "RUNNING" || status === "RUNNING_CRASH";
};
...
const BatchExecutionDetail: React.FC<Props> = ({
  ...
  const progressPercent = batch.progressPercent ?? 0;
  const remainingSeconds = batch.remainingSeconds ?? 0;
  ...
  return (
    <div className="detail-panel">
      ...
      >
        ...
        <span className={`batch-tag tag-${batch.status.toLowerCase()}`}>
          {batch.status}
        </span>
      </div>
      ...
      <div className="detail-section">
        ...
        <div className="info-grid">
          <div className="info-item">
            <label>Created</label>
            <span>{new Date(batch.createTime).toLocaleString()}</span>
          </div>

          {batch.startTime && (
            <div className="info-item">
              <label>Started</label>
              <span>{new Date(batch.startTime).toLocaleString()}</span>
            </div>
          )}

          {batch.estimatedEndTime && isRunningStatus(batch.status) && (
            <div className="info-item">
              <label>Estimated End</label>
              <span>{new Date(batch.estimatedEndTime).toLocaleString()}</span>
            </div>
          )}
          ...
          {batch.endTime && (
            <div className="info-item">
              ...
              <span>{new Date(batch.endTime).toLocaleString()}</span>
            </div>
          )}
        </div>
      </div>
      ...
      {isRunningStatus(batch.status) && (
        <div className="detail-section">
          ...
          <div className="wip-progress-wrap">
            <div className="wip-progress-info">
              <span>Processing</span>
              <span>{progressPercent}%</span>
            </div>

            <div className="wip-progress-bar-bg">
              <div
      ...
      <div className="detail-section" style={{ flex: 1 }}>
        ...
        <div className="barcode-list">
          {batch.sampleBarcodes.map((code) => (
            <span key={code} className="barcode-pill">
              {code}
            </span>
          ))}
        </div>
      ...
      <div className="detail-actions">
        ...
        {isRunningStatus(batch.status) && (
          ...
          >
            Equipment is running. Waiting for automatic result...
          </div>
        )}
