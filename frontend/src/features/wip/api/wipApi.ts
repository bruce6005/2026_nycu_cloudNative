...
export async function fetchWIPBatches(): Promise<WIPBatchDTO[]> {
  const res = await fetch(`${CONFIG.API_BASE}/api/wip`);
  return res.json();
}

export async function startWIPBatch(id: number): Promise<WIPBatchDTO> {
  const res = await fetch(`${CONFIG.API_BASE}/api/wip/${id}/start`, {
    method: "PATCH",
  });
...
export async function finishWIPBatch(id: number): Promise<WIPBatchDTO> {
  const res = await fetch(`${CONFIG.API_BASE}/api/wip/${id}/finish`, {
    method: "PATCH",
  });
